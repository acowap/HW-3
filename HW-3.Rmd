...
bitcoin_vol <- bitcoin %>% mutate(Avglag = lag(Avg)) %>% mutate(volatility_b = (Avg-Avglag)/Avg)

ggplot(data = bitcoin_vol, aes(x =Date, y=volatility_b)) + geom_line() + 
  labs(title = "Bitcoin Volatility", y="Volatility")

gold_vol <- gold %>% mutate(Valuelag = lag(Value)) %>% mutate(volatility_g = (Value-Valuelag)/Value)
